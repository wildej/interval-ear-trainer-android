...
- **Основной источник:** `app/src/main/assets/soundfonts/Chorium.SF2` — рендер через [fluidsynth-kmp](https://github.com/kotlinds/fluidsynth-kmp) (FluidSynth, **LGPL-2.1** для нативных библиотек; см. лицензию пакета на Maven / репозитории). При первом запуске файл копируется во внутреннее хранилище приложения. **Лицензия на сам SF2** не как у кода проекта: у линейки Chorium разные сборки, см. раздел **Chorium** в [`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md).
