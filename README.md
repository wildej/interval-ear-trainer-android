...
- По умолчанию используется файл `app/src/main/assets/audio/piano_ref.wav` (моно 16-bit PCM), записанный на **C4** (~261.63 Hz). Остальные ноты получаются ресемплингом.
- Чтобы заменить тембр, подмени этот WAV на свой (тот же формат; желательно та же опорная нота или поправь `referenceRootHz` в `AssetSampleIntervalAudioPlayer`).
- Если файла нет, включается запасной синтезатор (синус).

## Тайминг воспроизведения

- Паузы и длительность ноты настраиваются в `IntervalPlaybackTiming`.
